...
def _append_csv(csv_file_path: Path, data: list[list[str]]):
...
def _validate_csv_rows(src_csv_rows: list[dict], csv_rows_from_str: list[dict]) -> bool:
    ...
    if src_ids != str_ids:
        logger.error(f"ids do not match: {src_ids} != {str_ids}")
        return False
    return True
...
def append_csv_rows(csv_file: DeckCsv, csv_rows: list[dict]):
    ...
    _append_csv(csv_file.file_path, list_data)
...
def append_error_csv_rows(csv_file: DeckCsv, csv_rows: list[dict]):
    if not os.path.exists(csv_file.error_file_path):
        _clear_csv(csv_file.error_file_path)
        _append_csv(csv_file.error_file_path, [csv_file.columns_values])
    list_data = _convert_to_list(csv_rows, csv_file.columns)
    _append_csv(csv_file.error_file_path, list_data)
